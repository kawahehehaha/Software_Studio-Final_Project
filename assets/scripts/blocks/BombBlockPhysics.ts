...
cc.Class({
    ...
    spawnBomb: function () {
        ...
        } else if (this.bombFrame) {
            bomb = new cc.Node('Bomb');
            var sprite = bomb.addComponent(cc.Sprite);
            sprite.spriteFrame = this.bombFrame;

            if (this.bombFrame.getOriginalSize) {
                bomb.setContentSize(this.bombFrame.getOriginalSize());
            }

            bomb.addComponent(cc.RigidBody);
            var collider = bomb.addComponent(cc.PhysicsBoxCollider);
            collider.size = bomb.getContentSize();
            bomb.addComponent('BombPowerup');
        }
        ...
        bomb.setPosition(
            this.node.x,
            this.node.y + this.bombOffsetY * verticalDirection
        );

        var bombController = bomb.getComponent('BombPowerup');
        if (bombController && bombController.reveal) {
            bombController.reveal(verticalDirection);
        }
    },
